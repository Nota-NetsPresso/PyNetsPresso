...
class ConverterV1:
    ...
    def _download_converted_model(
        self, conversion_task: ConversionTask, local_path: str
    ) -> None:
    ...
    def convert_model(
        ...
        try:
            ...
            metadata = MetadataHandler.init_metadata(
                folder_path=output_dir, task_type=TaskType.CONVERT
            )
            ...
            if type(model) is Model:
                ...
                target_device_name in DeviceName.JETSON_DEVICES
                and target_software_version is None
            ):
            ...
            if target_device_name in DeviceName.JETSON_DEVICES:
                devices = (
                    TargetDeviceFilter.filter_devices_with_device_software_version(
            ...
            metadata.update_converted_model_path(
                converted_model_path=default_model_path.with_suffix(
                    extension
                ).as_posix()
            )
            ...
            metadata.update_available_devices(
                converter_uploaded_model.available_devices
            )
